# TIPOLÓGIA PRÁCTICA 2

## Integrantes del grupo
- Cristopher Valenzuela Espinoza
- Lydia 

## Descripción
Este repositorio contiene las soluciones de la práctica de tipología de datos.  
El objetivo es analizar la relación entre la incidencia de Infecciones Respiratorias Agudas (IRAs), la gripe y el polen acumulado, aplicando técnicas de limpieza de datos, exploración, modelado supervisado (Random Forest), clustering (K-Means) y pruebas de contraste de hipótesis.


## Estructura del repositorio
├── README.md                       # Descripción del proyecto y nombres del grupo
├── src/                          # Carpeta con el código de análisis
│   └── PRACTICA2_tipologia.ipynb
├── dataset_integrado_gold.xls 
├── dataset_integrado_gold_limpio.xls
├── evolucion_de_los_casos_de_gripe_en_españa_por_semanas.xls # tasa gripe
├── evolucion_de_los_casos_de_infecciones_respiratorias_agudas_(iras)_en_españa (1).xls # tasa IRAS
├── polen_acumulado_2025_todas_estaciones.xls # Polen


